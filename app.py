...
# ════════════════════════════════════════════════════════════════════════════
    ...
    for i, (tab, year) in enumerate(zip(tabs, years)):
        with tab:
            ...
            with uc1:
                st.markdown(f"<div style='font-size:0.82rem;color:#8FA3BF;margin-bottom:0.3rem;'>📄 Bank Statement — FY {year}-{str(year+1)[2:]}</div>", unsafe_allow_html=True)
            ...
            with uc2:
                st.markdown(f"<div style='font-size:0.82rem;color:#8FA3BF;margin-bottom:0.3rem;'>📋 ITR / Form-16 — FY {year}-{str(year+1)[2:]}</div>", unsafe_allow_html=True)
            ...
            if inc > 0:
                ...
                <div style='background:#0A1628;border:1px solid #1E3A5F;border-radius:8px;padding:0.7rem 1rem;
                            margin-top:0.5rem;font-size:0.82rem;color:#8FA3BF;display:flex;gap:1.5rem;flex-wrap:wrap;'>
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    <div style='display:flex;align-items:center;gap:0.8rem;margin-bottom:1.5rem;'>
        <span style='font-size:0.82rem;color:#8FA3BF;'>Rate data: {rates_data['source']}</span>
        {live_badge}
    </div>
    """, unsafe_allow_html=True)
    ...
    with tab_home:
        ...
        with col_hr:
            ...
            fig_h = go.Figure(go.Indicator(
                mode="gauge+number",
                value=hd["avg"],
                title={"text": "Avg Home Loan Rate (%)", "font": {"size": 13, "color": "#8FA3BF"}},
                number={"suffix": "% p.a.", "font": {"size": 32, "color": "#F0B429"}},
                gauge={
                    "axis": {"range": [7, 12], "tickfont": {"color": "#8FA3BF", "size": 9}},
            ...
            st.markdown(f"""
            <div style='text-align:center;font-size:0.8rem;color:#8FA3BF;background:#112240;border:1px solid #1E3A5F;border-radius:8px;padding:0.6rem;'>
                RBI Repo Rate: <b style='color:#F0B429;'>{rates_data['repo']}%</b> &nbsp;|&nbsp;
                Spread: <b style='color:#0EC4B0;'>~2.0–3.5%</b><br>
                Market Range: <b style='color:#E8EDF5;'>{hd['low']}% – {hd['high']}%</b>
            </div>
            """, unsafe_allow_html=True)
    ...
    with tab_pers:
        ...
        with col_pr:
            fig_p = go.Figure(go.Indicator(
                mode="gauge+number",
                value=pd_r["avg"],
                title={"text": "Avg Personal Loan Rate (%)", "font": {"size": 13, "color": "#8FA3BF"}},
                number={"suffix": "% p.a.", "font": {"size": 32, "color": "#E05C5C"}},
                gauge={
                    "axis": {"range": [10, 36], "tickfont": {"color": "#8FA3BF", "size": 9}},
            ...
            st.markdown(f"""
            <div style='text-align:center;font-size:0.8rem;color:#8FA3BF;background:#112240;border:1px solid #1E3A5F;border-radius:8px;padding:0.6rem;'>
                Market Range: <b style='color:#E8EDF5;'>{pd_r['low']}% – {pd_r['high']}%</b><br>
        ...
        if not scores_ready:
            ...
            with col_inp:
                ...
                <div style='background:rgba(240,180,41,0.08);border:1px solid rgba(240,180,41,0.3);border-radius:8px;padding:0.8rem;font-size:0.85rem;color:#F0B429;'>
                    ...
                    <span style='font-size:0.75rem;color:#8FA3BF;'>Determined by FinBridge Score {fb} within {rate_lo}–{rate_hi}% range</span>
                </div>
                """, unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    with col_score:
        ...
        <div class="score-hero">
            ...
            <div style='font-size:0.8rem;color:#8FA3BF;margin-top:0.3rem;'>
                Assessed for: <b style="color:#E8EDF5">{prof.get("name","Applicant")}</b>
            </div>
    ...
    with col_r:
        ...
        fig_radar.update_layout(
            ...
            showlegend=True, legend=dict(font=dict(color="#8FA3BF", size=10)),
        ...
        <div class='explain-body'>
            ...
            <table style='width:100%;border-collapse:collapse;margin-top:0.8rem;font-size:0.82rem;'>
                <tr style='border-bottom:1px solid #1E3A5F;'>
                    <th style='text-align:left;padding:0.4rem 0.8rem;color:#8FA3BF;'>Loan Type</th>
                    <th style='text-align:center;padding:0.4rem;color:#8FA3BF;'>CIBIL Eligibility</th>
                    <th style='text-align:center;padding:0.4rem;color:#F0B429;'>FinBridge Eligibility</th>
                </tr>
                <tr style='border-bottom:1px solid #0A1628;'>
                    <td style='padding:0.5rem 0.8rem;color:#E8EDF5;'>Home Loan</td>
                ...
                <tr style='border-bottom:1px solid #0A1628;'>
                    <td style='padding:0.5rem 0.8rem;color:#E8EDF5;'>Personal Loan</td>
                ...
                <tr style='border-bottom:1px solid #0A1628;'>
                    <td style='padding:0.5rem 0.8rem;color:#E8EDF5;'>Business / MSME Loan</td>
                    <td style='text-align:center;padding:0.5rem;color:{"#3EC87A" if ci>=700 else "#E05C5C"};'>{"✅ Eligible" if ci>=700 else "⚠️ Marginal" if ci>=650 else "❌ Unlikely"}</td>
                ...
                <tr>
                    <td style='padding:0.5rem 0.8rem;color:#E8EDF5;'>Gold / Secured Loan</td>
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    fig_comp.add_trace(go.Bar(name="Traditional Score", x=["Score"], y=[ci],  marker_color=color_ci, width=0.25,
                               text=[ci], textposition="outside", textfont=dict(color="#E8EDF5", size=14)))
    fig_comp.add_trace(go.Bar(name="FinBridge Score",   x=["Score"], y=[fb],  marker_color=color_fb, width=0.25,
                               text=[fb], textposition="outside", textfont=dict(color="#E8EDF5", size=14)))
    ...
    header_html = "<div style='display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:8px;'>" + \
        "".join(f"<div style='background:#0A1628;border:1px solid #1E3A5F;border-radius:8px;padding:0.7rem 1rem;font-size:0.8rem;font-weight:600;color:#8FA3BF;text-transform:uppercase;letter-spacing:0.05em;'>{h}</div>"
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    with col2:
        fig_pie = go.Figure(go.Pie(
            ...
            textinfo="label+percent", textfont=dict(color="#E8EDF5", size=11),
    ...
    <div style='background:rgba(0,0,0,0.2);border:2px solid {net_color};border-radius:10px;padding:1rem 1.3rem;margin-top:1rem;'>
        ...
        <div style='font-size:0.8rem;color:#8FA3BF;margin-top:0.4rem;'>{note}</div>
    </div>
    """, unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    <div style='background:linear-gradient(135deg,#1A3A5C 0%,#0A1628 100%);border:1px solid #F0B429;border-radius:14px;padding:2rem;margin-bottom:1.5rem;'>
        <div style='display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:1rem;'>
            <div>
                ...
                <div style='color:#8FA3BF;font-size:0.82rem;margin-top:0.3rem;'>Confidential | Generated: {datetime.now().strftime("%B %Y")}</div>
            </div>
    ...
    for col, (lbl, val, clr) in zip([r1,r2,r3,r4,r5], [
        ...
        ("Total Bounces",      str(tot_bnc),          "#E05C5C" if tot_bnc > 2 else "#8FA3BF"),
    ]):
        with col:
