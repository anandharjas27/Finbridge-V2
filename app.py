...
with st.sidebar:
    ...
    if st.session_state.scores_computed:
        ...
        <div style='text-align:center;padding:1rem;background:#112240;border-radius:10px;border:1px solid #1E3A5F;'>
            <div style='font-size:0.7rem;color:#BA55D3;letter-spacing:0.1em;text-transform:uppercase;'>FinBridge Score</div>
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    for i, (tab, year) in enumerate(zip(tabs, years)):
        with tab:
            ...
            with uc1:
                st.markdown(f"<div style='font-size:0.82rem;color:#BA55D3;margin-bottom:0.3rem;'>📄 Bank Statement — FY {year}-{str(year+1)[2:]}</div>", unsafe_allow_html=True)
            ...
            with uc2:
                st.markdown(f"<div style='font-size:0.82rem;color:#BA55D3;margin-bottom:0.3rem;'>📋 ITR / Form-16 — FY {year}-{str(year+1)[2:]}</div>", unsafe_allow_html=True)
            ...
            if inc > 0:
                ...
                <div style='background:#0A1628;border:1px solid #1E3A5F;border-radius:8px;padding:0.7rem 1rem;
                            margin-top:0.5rem;font-size:0.82rem;color:#BA55D3;display:flex;gap:1.5rem;flex-wrap:wrap;'>
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    <div style='display:flex;align-items:center;gap:0.8rem;margin-bottom:1.5rem;'>
        <span style='font-size:0.82rem;color:#BA55D3;'>Rate data: {rates_data['source']}</span>
        {live_badge}
    </div>
    """, unsafe_allow_html=True)
    ...
    with tab_home:
        ...
        with col_hr:
            ...
            fig_h = go.Figure(go.Indicator(
                mode="gauge+number",
                value=hd["avg"],
                title={"text": "Avg Home Loan Rate (%)", "font": {"size": 13, "color": "#BA55D3"}},
                number={"suffix": "% p.a.", "font": {"size": 32, "color": "#F0B429"}},
                gauge={
                    "axis": {"range": [7, 12], "tickfont": {"color": "#BA55D3", "size": 9}},
            ...
            st.markdown(f"""
            <div style='text-align:center;font-size:0.8rem;color:#BA55D3;background:#112240;border:1px solid #1E3A5F;border-radius:8px;padding:0.6rem;'>
                RBI Repo Rate: <b style='color:#F0B429;'>{rates_data['repo']}%</b> &nbsp;|&nbsp;
                Spread: <b style='color:#0EC4B0;'>~2.0–3.5%</b><br>
                Market Range: <b style='color:#BA55D3;'>{hd['low']}% – {hd['high']}%</b>
            </div>
            """, unsafe_allow_html=True)
    ...
    with tab_pers:
        ...
        with col_pr:
            fig_p = go.Figure(go.Indicator(
                mode="gauge+number",
                value=pd_r["avg"],
                title={"text": "Avg Personal Loan Rate (%)", "font": {"size": 13, "color": "#BA55D3"}},
                number={"suffix": "% p.a.", "font": {"size": 32, "color": "#E05C5C"}},
                gauge={
                    "axis": {"range": [10, 36], "tickfont": {"color": "#BA55D3", "size": 9}},
            ...
            st.markdown(f"""
            <div style='text-align:center;font-size:0.8rem;color:#BA55D3;background:#112240;border:1px solid #1E3A5F;border-radius:8px;padding:0.6rem;'>
                Market Range: <b style='color:#BA55D3;'>{pd_r['low']}% – {pd_r['high']}%</b><br>
        ...
        if not scores_ready:
            ...
            with col_inp:
                ...
                <div style='background:#0A1628;border:1px solid #1E3A5F;border-radius:8px;padding:0.8rem;margin:0.8rem 0;font-size:0.82rem;'>
                    <b style='color:#F0B429;'>Industry:</b> <span style='color:#BA55D3;'>{industry}</span><br>
                    <b style='color:#F0B429;'>Market Rate Range:</b> <span style='color:#BA55D3;'>{rate_lo:.1f}% – {rate_hi:.1f}% p.a.</span><br>
                    <b style='color:#F0B429;'>Max Tenure:</b> <span style='color:#BA55D3;'>Up to {max_tenure_yr} years</span>
                </div>
                <div style='background:rgba(240,180,41,0.08);border:1px solid rgba(240,180,41,0.3);border-radius:8px;padding:0.8rem;font-size:0.85rem;color:#F0B429;'>
                    ...
                    <span style='font-size:0.75rem;color:#BA55D3;'>Determined by FinBridge Score {fb} within {rate_lo}–{rate_hi}% range</span>
                </div>
                """, unsafe_allow_html=True)
            ...
            with col_res:
                ...
                <div style='margin:1rem 0;'>
                    <div style='display:flex;justify-content:space-between;font-size:0.8rem;color:#BA55D3;margin-bottom:0.3rem;'>
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    with col_score:
        ...
        <div class="score-hero">
            <div style='font-size:0.8rem;color:#BA55D3;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:0.5rem;'>FinBridge Credit Score</div>
            <div class="score-num">{fb}</div>
            <div style='margin:0.8rem 0;'>
                <span class='badge {badge}' style='font-size:0.95rem;padding:0.4rem 1.2rem;'>{label}</span>
            </div>
            <div style='font-size:0.8rem;color:#BA55D3;'>Score Range: 300 – 900</div>
            <div style='font-size:0.8rem;color:#BA55D3;margin-top:0.3rem;'>
                Assessed for: <b style="color:#BA55D3">{prof.get("name","Applicant")}</b>
            </div>
            <div style='font-size:0.8rem;color:#BA55D3;'>Profile: {prof.get("profile_type","")}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col_gauge:
        ...
        fig_gauge.update_layout(
            ...
            font={"color": "#BA55D3"}, margin=dict(t=40, b=10, l=30, r=30),
        )
    ...
    with col_r:
        ...
        fig_radar.update_layout(
            polar=dict(bgcolor="rgba(0,0,0,0)",
                       radialaxis=dict(visible=True, range=[0, 25], gridcolor="#1E3A5F", tickfont=dict(color="#BA55D3", size=9)),
                       angularaxis=dict(tickfont=dict(color="#BA55D3", size=10), gridcolor="#1E3A5F")),
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            showlegend=True, legend=dict(font=dict(color="#BA55D3", size=10)),
    ...
    with col_b2:
        ...
        fig_bar.add_trace(go.Bar(x=cats, y=vals, name="Your Score", marker_color=colors_bar,
                                  ...
                                  textfont=dict(color="#BA55D3", size=10)))
        fig_bar.update_layout(
            barmode="overlay", paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            font=dict(color="#BA55D3", size=10),
            xaxis=dict(gridcolor="#1E3A5F", tickfont=dict(color="#BA55D3", size=9)),
            yaxis=dict(gridcolor="#1E3A5F"),
            legend=dict(font=dict(color="#BA55D3", size=10)),
    ...
    fig_trend.update_layout(
        ...
        font=dict(color="#BA55D3"),
        xaxis=dict(gridcolor="#1E3A5F", title="Financial Year"),
        yaxis=dict(gridcolor="#1E3A5F", title="Amount (₹)"),
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1, font=dict(color="#BA55D3", size=10)),
        ...
        <div class='explain-body'>
            ...
            <table style='width:100%;border-collapse:collapse;margin-top:0.8rem;font-size:0.82rem;'>
                <tr style='border-bottom:1px solid #1E3A5F;'>
                    <th style='text-align:left;padding:0.4rem 0.8rem;color:#BA55D3;'>Loan Type</th>
                    <th style='text-align:center;padding:0.4rem;color:#BA55D3;'>CIBIL Eligibility</th>
                    <th style='text-align:center;padding:0.4rem;color:#F0B429;'>FinBridge Eligibility</th>
                </tr>
                <tr style='border-bottom:1px solid #0A1628;'>
                    <td style='padding:0.5rem 0.8rem;color:#BA55D3;'>Home Loan</td>
                ...
                <tr style='border-bottom:1px solid #0A1628;'>
                    <td style='padding:0.5rem 0.8rem;color:#BA55D3;'>Personal Loan</td>
                ...
                <tr style='border-bottom:1px solid #0A1628;'>
                    <td style='padding:0.5rem 0.8rem;color:#BA55D3;'>Business / MSME Loan</td>
                    <td style='text-align:center;padding:0.5rem;color:{"#3EC87A" if ci>=700 else "#E05C5C"};'>{"✅ Eligible" if ci>=700 else "⚠️ Marginal" if ci>=650 else "❌ Unlikely"}</td>
                ...
                <tr>
                    <td style='padding:0.5rem 0.8rem;color:#BA55D3;'>Gold / Secured Loan</td>
    ...
    msg, col = interpretation.get(label, ("", "#BA55D3"))
    st.markdown(f"""
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    with col_ci:
        ...
        <div style='text-align:center;padding:2rem 1rem;background:#112240;border:1px solid #1E3A5F;border-radius:14px;'>
            <div style='font-size:0.7rem;letter-spacing:0.15em;text-transform:uppercase;color:#BA55D3;margin-bottom:0.5rem;'>Traditional CIBIL Score</div>
            ...
            <div style='font-size:0.75rem;color:#BA55D3;margin-top:1rem;'>Bureau-based | Payment History Focused</div>
        </div>
        """, unsafe_allow_html=True)
    with col_fb:
        st.markdown(f"""
        <div style='text-align:center;padding:2rem 1rem;background:linear-gradient(135deg,#1A3A5C,#0A1628);border:2px solid #F0B429;border-radius:14px;'>
            ...
            <div style='font-size:0.75rem;color:#BA55D3;margin-top:1rem;'>Bank Statement Driven | Cash-Flow Intelligence</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    fig_comp.add_trace(go.Bar(name="Traditional Score", x=["Score"], y=[ci],  marker_color=color_ci, width=0.25,
                               text=[ci], textposition="outside", textfont=dict(color="#BA55D3", size=14)))
    fig_comp.add_trace(go.Bar(name="FinBridge Score",   x=["Score"], y=[fb],  marker_color=color_fb, width=0.25,
                               text=[fb], textposition="outside", textfont=dict(color="#BA55D3", size=14)))
    ...
    fig_comp.update_layout(
        ...
        font=dict(color="#BA55D3"),
        legend=dict(font=dict(color="#BA55D3"), orientation="h", y=1.1),
    ...
    header_html = "<div style='display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:8px;'>" + \
        "".join(f"<div style='background:#0A1628;border:1px solid #1E3A5F;border-radius:8px;padding:0.7rem 1rem;font-size:0.8rem;font-weight:600;color:#BA55D3;text-transform:uppercase;letter-spacing:0.05em;'>{h}</div>"
                for h in ["Parameter", "Traditional CIBIL", "FinBridge"]) + "</div>"
    rows_html = ""
    for param, trad, fb_val in comp_data:
        rows_html += f"""
        <div style='display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:6px;'>
            <div style='background:#112240;border:1px solid #1E3A5F;border-radius:6px;padding:0.6rem 0.9rem;font-size:0.82rem;color:#BA55D3;font-weight:500;'>{param}</div>
            <div style='background:#112240;border:1px solid #1E3A5F;border-radius:6px;padding:0.6rem 0.9rem;font-size:0.82rem;color:#BA55D3;'>{trad}</div>
            <div style='background:linear-gradient(135deg,#1A3A5C,#0A1628);border:1px solid rgba(240,180,41,0.3);border-radius:6px;padding:0.6rem 0.9rem;font-size:0.82rem;color:#BA55D3;'>{fb_val}</div>
        </div>"""
    ...
    for col, (icon, title, desc) in zip(benefit_cols, benefits):
        with col:
            ...
            <div class='metric-card' style='padding:1.3rem;'>
                ...
                <div style='font-size:0.92rem;font-weight:600;color:#BA55D3;margin:0.5rem 0 0.3rem;'>{title}</div>
                <div style='font-size:0.78rem;color:#BA55D3;'>{desc}</div>
            </div>
            """, unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    with col2:
        fig_pie = go.Figure(go.Pie(
            ...
            textinfo="label+percent", textfont=dict(color="#BA55D3", size=11),
            hovertemplate="%{label}: ₹%{value:,.0f}<extra></extra>"
        ))
        fig_pie.add_annotation(text=f"Total<br>{format_inr(total_pmt)}", x=0.5, y=0.5, showarrow=False,
                               font=dict(size=13, color="#BA55D3"))
        fig_pie.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                              showlegend=True, legend=dict(font=dict(color="#BA55D3"), orientation="h", y=-0.1),
        ...
        fig_amort.update_layout(
            ...
            legend=dict(font=dict(color="#BA55D3", size=9), orientation="h", y=1.1),
            font=dict(color="#BA55D3", size=9),
    ...
    <div style='background:rgba(0,0,0,0.2);border:2px solid {net_color};border-radius:10px;padding:1rem 1.3rem;margin-top:1rem;'>
        ...
        <div style='font-size:0.8rem;color:#BA55D3;margin-top:0.4rem;'>{note}</div>
    </div>
    """, unsafe_allow_html=True)
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    <div style='background:linear-gradient(135deg,#1A3A5C 0%,#0A1628 100%);border:1px solid #F0B429;border-radius:14px;padding:2rem;margin-bottom:1.5rem;'>
        <div style='display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:1rem;'>
            <div>
                ...
                <div style='color:#BA55D3;font-size:0.82rem;margin-top:0.3rem;'>Confidential | Generated: {datetime.now().strftime("%B %Y")}</div>
            </div>
            <div style='text-align:right;'>
                <div style='font-size:0.75rem;color:#BA55D3;'>FinBridge Score</div>
        ...
        <div style='display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;font-size:0.82rem;'>
            <div><span style='color:#BA55D3;'>Name:</span> <span style='color:#BA55D3;'>{prof.get("name","N/A")}</span></div>
            <div><span style='color:#BA55D3;'>Age:</span> <span style='color:#BA55D3;'>{prof.get("age","N/A")}</span></div>
            <div><span style='color:#BA55D3;'>Profile:</span> <span style='color:#BA55D3;'>{prof.get("profile_type","N/A")}</span></div>
            <div><span style='color:#BA55D3;'>City:</span> <span style='color:#BA55D3;'>{prof.get("city","N/A")}</span></div>
            <div><span style='color:#BA55D3;'>Occupation:</span> <span style='color:#BA55D3;'>{prof.get("occupation","N/A")}</span></div>
            <div><span style='color:#BA55D3;'>Credit Cards:</span> <span style='color:#BA55D3;'>{prof.get("credit_cards","N/A")}</span></div>
            <div><span style='color:#BA55D3;'>Active Loans:</span> <span style='color:#BA55D3;'>{prof.get("loans_active","N/A")}</span></div>
            <div><span style='color:#BA55D3;'>Collateral:</span> <span style='color:#BA55D3;'>{prof.get("has_collateral","N/A")}</span></div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    for col, (lbl, val, clr) in zip([r1,r2,r3,r4,r5], [
        ...
        ("Total Bounces",      str(tot_bnc),          "#E05C5C" if tot_bnc > 2 else "#BA55D3"),
    ]):
        with col:
    ...
    for comp_name, score_val in comps.items():
        ...
        <div style='display:grid;grid-template-columns:180px 1fr 80px 70px;gap:1rem;align-items:center;margin-bottom:0.6rem;'>
            <div style='font-size:0.85rem;color:#BA55D3;'>{comp_name}</div>
    ...
    with col_str:
        ...
        for s in strengths:
            st.markdown(f"""<div style='background:rgba(62,200,122,0.05);border:1px solid rgba(62,200,122,0.2);border-radius:8px;padding:0.6rem 0.9rem;margin-bottom:0.5rem;font-size:0.83rem;color:#BA55D3;'>✅ {s}</div>""", unsafe_allow_html=True)
    ...
    with col_imp:
        ...
        for im in improvements:
            st.markdown(f"""<div style='background:rgba(240,180,41,0.05);border:1px solid rgba(240,180,41,0.2);border-radius:8px;padding:0.6rem 0.9rem;margin-bottom:0.5rem;font-size:0.83rem;color:#BA55D3;'>⚠️ {im}</div>""", unsafe_allow_html=True)
